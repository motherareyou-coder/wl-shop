			...
			<template #default="{ row: coupon }: { row: CouponTemplate }">
				...
				>
					<div class="user-coupon__item in">
						...
						<p v-if="coupon.validityType === 1" class="user-coupon__item--time">
							<!-- 格式化时间为年月日 -->
							{{ $t('Expiry') }}：
						...
						<p v-if="coupon.validityType === 2" class="user-coupon__item--time">
							<!-- 有效期：领取后x天内可用 -->
...
<style lang="scss">
@import url('~/pages/user/coupon/index.scss');
</style>
