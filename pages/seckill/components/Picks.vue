...
<template>
	<main v-infinite-scroll="load" class="daily-picks-area__main" role="grid">
		<div
			v-for="item in data" :key="item.id" class="daily-picks-area__list daily-picks-area__list--doing"
			...
			<div class="daily-picks-area__item">
				<article class="daily-picks-area__item-detail">
					...
					<div class="flex items-center">
						<span>{{ $t('limit') }}</span>
						<el-progress
							class="w-2/4 ml-2" :percentage="(100 * (item.totalStock / item.stock)) / item.totalStock"
